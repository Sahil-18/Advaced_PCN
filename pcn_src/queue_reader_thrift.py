...
s1_queue_file= './Results/s1_queue_lengths.csv'
s2_queue_file= './Results/s2_queue_lengths.csv'
s1_threshold_file= './Results/s1_threshold.csv'
...
def main(thrift_port_s1, thrift_port_s2):
    ...
    # Create Thrift connections to the switches
        ...
        s1_thresh_file = open(s1_threshold_file, 'w')
        ...
        s1_thresh_file.write('Time (in HH:MM:SS) ,Time (s), Threshold\n')
        s2_thresh_file.write('Time (in HH:MM:SS) ,Time (s), Threshold\n')

        # Open files for writing queue lengths
        start_time = time()
        while True:
            readQueueLengths(s1, 1, s1_file, s1_thresh_file)  # Read queue length for port 1 on switch s1
            readQueueLengths(s2, 1, s2_file, s2_thresh_file)  # Read queue length for port 2 on switch s2
