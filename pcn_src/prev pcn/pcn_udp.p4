...
control MyIngress(inout headers hdr,
    ...
    action handle_pcn_start (egreessSpec_t port) {
        ...
        if (pcn_enabled == 0) {
            ...
            if (THRESHOLD_SCHEME == MIN_THRESHOLD) {
                ...
                // Approximate using integer math to avoid divide-by-zero
    ...
    */
    ...
    action compute_hash(ip4Addr_t srcAddr, ip4Addr_t dstAddr) {
    ...
    apply {
        if (hdr.ipv4.isValid()) {
            ...
            compute_hash(hdr.ipv4.srcAddr, hdr.ipv4.dstAddr);
